...
Iterable<double> _getXOffsets(WidgetTester tester) {
  return tester
      .renderObject<RenderWrap2>(find.byType(Wrap2))
      .getChildrenAsList()
      .map((RenderBox e) => e.localToGlobal(Offset.zero).dy);
}
...
void _expectAllTheSame(Iterable<dynamic> items) {
  expect(items,
      List<dynamic>.generate(items.length, (int index) => items.firstOrNull));
}
