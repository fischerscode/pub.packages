...
/// See also:
///
///  * [RenderWrap2], the wrap render object.
...
/// Parent data for use with [RenderWrap2].
class Wrap2ParentData extends ContainerBoxParentData<RenderBox> {
  int _runIndex = 0;
...
///
/// A [RenderWrap2] lays out each child and attempts to place the child adjacent
...
/// [RenderWrap2] creates a new _run_ adjacent to the existing children in the
/// cross axis.
///
...
class RenderWrap2 extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, Wrap2ParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, Wrap2ParentData> {
  ...
  /// grouped together in the center of the overall [RenderWrap2] in the cross
  /// axis.
  ///
  /// Defaults to [WrapAlignment.start].
  ///
  ...
  ///
  /// If there is additional free space in the overall [RenderWrap2] (e.g.,
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! Wrap2ParentData) {
      child.parentData = Wrap2ParentData();
    }
  }
  ...
  double _getMainAxisExtent(Size childSize) =>
      _getMainAxisExtentInDirection(childSize, direction);
  ...
  double _getCrossAxisExtent(Size childSize) =>
      _getCrossAxisExtentInDirection(childSize, direction);
  ...
  Size computeDryLayout(covariant BoxConstraints constraints) {
    final (:double crossAxisExtent, :double mainAxisExtent, runCount: _) =
        _computeLayout(
      layoutChild: ChildLayoutHelper.dryLayoutChild,
      next: (RenderBox child, Wrap2ParentData parentData) => childAfter(child),
      direction: direction,
  ...
  void performLayout() {
    ...
    final (:double crossAxisExtent, :double mainAxisExtent, :int runCount) =
        _computeLayout(
      layoutChild: (RenderBox child, BoxConstraints constraints) {
        child.layout(constraints, parentUsesSize: true);
        return child.size;
      },
      next: (RenderBox child, Wrap2ParentData parentData) =>
          parentData.nextSibling,
      onRunFinished: (
          {required int childCount,
          required double runCrossAxisExtent,
          required double runMainAxisExtent}) {
        runMetrics.add(
            _RunMetrics(runMainAxisExtent, runCrossAxisExtent, childCount));
      },
      onLayoutChild: (Wrap2ParentData parentData, int runIndex) {
  ...
  static ({double mainAxisExtent, double crossAxisExtent, int runCount})
      _computeLayout({
    required ChildLayouter layoutChild,
    required RenderBox? Function(RenderBox child, Wrap2ParentData parentData)
        next,
    void Function(
            {required double runMainAxisExtent,
            required double runCrossAxisExtent,
            required int childCount})?
        onRunFinished,
    void Function(Wrap2ParentData parentData, int runIndex)? onLayoutChild,
    ...
    final double Function(RenderBox child, double crossAxisSize)
        getChildMinIntrinsicMainAxisExtent;
    final BoxConstraints Function(double maxSize)
        childConstraintsFittingLooseInRun;
    final BoxConstraints Function(double maxSize)
    ...
    switch (direction) {
      case Axis.horizontal:
        ...
        childConstraintsTight = BoxConstraints(
            minWidth: constraints.maxWidth, maxWidth: constraints.maxWidth);
        mainAxisLimit = constraints.maxWidth;
        getChildMinIntrinsicMainAxisExtent =
            (RenderBox child, double crossAxisSize) =>
                child.getMinIntrinsicWidth(crossAxisSize);
        childConstraintsFittingLooseInRun =
            (double maxSize) => BoxConstraints(maxWidth: maxSize);
        childConstraintsFittingTightInRun = (double maxSize) =>
            BoxConstraints(minWidth: maxSize, maxWidth: maxSize);
      case Axis.vertical:
        childConstraintsLoose =
        ...
        getChildMinIntrinsicMainAxisExtent =
            (RenderBox child, double crossAxisSize) =>
                child.getMinIntrinsicHeight(crossAxisSize);
        childConstraintsFittingLooseInRun =
            (double maxSize) => BoxConstraints(maxHeight: maxSize);
        childConstraintsFittingTightInRun = (double maxSize) =>
            BoxConstraints(minHeight: maxSize, maxHeight: maxSize);
    }
    ...
    while (child != null) {
          ...
          if (runMainAxisExtent != 0 &&
            ...
            if (fit.isTight) {
              childSize = layoutChild(
                  child,
                  childConstraintsFittingTightInRun(
                      mainAxisLimit - runMainAxisExtent - spacing));
            } else {
              childSize = layoutChild(
                  child,
                  childConstraintsFittingLooseInRun(
            ...
            if (fit.isTight) {
              childSize = layoutChild(
                  child, childConstraintsFittingTightInRun(mainAxisLimit));
            } else {
              childSize = layoutChild(
      ...
      if (childCount > 0 &&
        ...
        if (onRunFinished != null) {
          onRunFinished(
              runMainAxisExtent: runMainAxisExtent,
              runCrossAxisExtent: runCrossAxisExtent,
