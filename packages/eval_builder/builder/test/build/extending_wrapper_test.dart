...
void main() {
  test('non wrapper', () async {
    var resolved = await resolveLibraryFromFile(
        'extending_wrapper_test_non_wrapped.dart', ['build']);
    ...
    expect(
        createPartFile('extending_wrapper_test_non_wrapped.dart', [
          ClassWrapperBuilder(
              element!,
              WrapperSettings(
                  bimodal: true,
        ...
        specMatches(
            matchesGolden(
                'extending_wrapper_test_non_wrapped.golden.dart', ['build']),
            PrefixResolver(element.library.prefixes)));
  });
  test('wrapper', () async {
    var resolved = await resolveLibraryFromFile(
        'extending_wrapper_test_wrapped.dart', ['build']);
    ...
    expect(element, isNotNull);
    expect(
        createPartFile('extending_wrapper_test_wrapped.dart', [
          ClassWrapperBuilder(
              base!,
              WrapperSettings(
                  bimodal: true,
        ...
        specMatches(
            matchesGolden(
                'extending_wrapper_test_wrapped.golden.dart', ['build']),
            PrefixResolver(element.library.prefixes)));
  });
}
